...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
                              ...
                              QTabWidget, QToolBar, QListWidget, QListWidgetItem, QListView) 
from PySide6.QtCore import Qt, QPoint, QRect, QSize, Signal, QTimer, QEvent, QCoreApplication, QObject, QPropertyAnimation, QEasingCurve, QRectF
from PySide6.QtGui import QPixmap, QImage, QPainter, QPen, QColor, QCursor, QAction, QPainterPath, QImageReader, QIcon
import cv2
...
class TunnelNX(QMainWindow):
    def __init__(self):
        ...
        self.auto_save_timer.setInterval(60000) # 60000毫秒 = 60秒
        self.auto_save_timer.timeout.connect(self.auto_save_node_graph)
        self.auto_save_timer.start()
        print("自动保存定时器已启动 (间隔60秒)")
    ...
    def open_image_path(self, file_path):
        ...
        try:
            ...
            # 1. 保存当前图像的节点图 (如果当前有图像和节点)
            if hasattr(self, 'current_image_path') and self.current_image_path and self.nodes:
                try:
                    current_image_filename = os.path.splitext(os.path.basename(self.current_image_path))[0]
                    main_graph_path = os.path.join(self.nodegraphs_folder, f"{current_image_filename}.json")
                    print(f"切换图片: 正在保存 '{current_image_filename}' 的主节点图到: {main_graph_path}")
                    # 保存主文件，但不抑制自动保存（允许记录撤销状态）
                    if not self.save_node_graph_to_file(main_graph_path):
                        print(f"警告: 保存当前主节点图失败: {main_graph_path}")
                    # 执行一次自动保存，以确保撤销堆栈是最新的
            ...
            expected_graph_path = os.path.join(self.nodegraphs_folder, f"{image_filename_no_ext}.json")

            loaded_existing = False
            if os.path.exists(expected_graph_path):
                print(f"找到关联节点图: {expected_graph_path}，尝试加载...")
                self.task_label.setText("正在加载关联节点图...")
                QApplication.processEvents()
                # 加载时也禁止自动保存
                if self.load_node_graph_from_file(expected_graph_path, suppress_auto_save=True):
                    print("成功加载关联节点图。")
                    # 确保加载后 current_image_path 仍然是我们打开的这个图像
                    self.current_image_path = dest_path if os.path.normpath(file_path) != os.path.normpath(dest_path) else file_path
                    loaded_existing = True
                    # 加载成功后，节点图处理和预览更新已在内部完成
                else:
                    print(f"加载关联节点图失败: {expected_graph_path}")
                    QMessageBox.warning(self, "加载警告", f"找到了关联的节点图文件，但加载失败:\n{expected_graph_path}\n将创建默认节点图。")
            else:
                print(f"未找到关联节点图: {expected_graph_path}")

            # 5. 如果没有找到或加载失败，创建默认节点图
            if not loaded_existing:
                print("创建默认节点图...")
                self.task_label.setText("创建默认节点图...")
                QApplication.processEvents()
            ...
            # 更新最终状态
            if loaded_existing:
                self.task_label.setText(f"已加载: {filename} 及其节点图")
            else:
                self.task_label.setText(f"已加载: {filename} (默认节点图)")
    ...
    def load_node_graph(self):
        ...
        if self.load_node_graph_from_file(file_path):
            # 处理节点图并更新预览
            self.process_node_graph()
            QMessageBox.information(self, "加载成功", "节点图已加载")
        else:
    ...
    def create_node_graph_area(self):
        ...
        # 创建节点图画布
        self.node_canvas_widget = QWidget()
        # self.node_canvas_widget.setStyleSheet("background-color: #000000;") # 改为使用主题背景色
        self.node_canvas_widget.setObjectName("node_canvas_widget") # 确保 objectName 被设置，以便 Aero 样式生效
        # self.node_canvas_widget.setStyleSheet("background-color: #D8E4EB;") # 或者直接设置颜色
    ...
    def draw_node(self, node):
        ...
        try:
            ...
            stylesheet_string = f"""
                ...
                QFrame[selected="true"] {{
                    ...
                    box-shadow: 0 0 8px {sel_shadow_color}; /* Lighter shadow when selected */
                }}
                QLabel {{
            ...
            stylesheet_string = f"""
                ...
                QFrame[selected="true"] {{
                    ...
                    box-shadow: 0 0 8px {fallback_shadow_color};
                }}
                QLabel {{
                    background-color: transparent;
                    color: {main_text_color};
        ...
        for i, input_type in enumerate(input_types):
            ...
            # 设置鼠标事件
            port_widget.mousePressEvent = lambda event, n=node, idx=i, t='input': self.on_port_click(event, n, idx, t)
            port_widget.setContextMenuPolicy(Qt.CustomContextMenu)
            port_widget.customContextMenuRequested.connect(
                lambda pos, n=node, idx=i, t='input': self.show_port_context_menu(pos, n, idx, t)
            )
        ...
        for i, output_type in enumerate(output_types):
            ...
            # 设置鼠标事件
            port_widget.mousePressEvent = lambda event, n=node, idx=i, t='output': self.on_port_click(event, n, idx, t)
            port_widget.setContextMenuPolicy(Qt.CustomContextMenu)
            port_widget.customContextMenuRequested.connect(
                lambda pos, n=node, idx=i, t='output': self.show_port_context_menu(pos, n, idx, t)
            )
...
# imgpp_REIMAGINED.py
    ...
    def show_node_preview_window(self, node):
        ...
        if node_id in self.node_preview_windows:
            ...
            if dialog and dialog.isVisible():
                print(f"Updating existing preview window for node {node_id}")
                # 获取最新的图像数据
                latest_img = None
                    ...
                    if target_node and target_port_type == 'input':
                         if source_node == target_node:
                             ...
                             try:
                                 ...
                                 if output_type == input_type or output_type == 'any' or input_type == 'any' or True:
                                     # 尝试连接
    ...
    def create_connection(self, output_node, output_port, input_node, input_port):
        ...
        # 连接创建会影响输入节点和其所有下游节点，所以需要选择性地重新计算
        self.process_node_graph(changed_nodes=[input_node])
        
        # 不直接调用 self.process_node_graph()
    ...
    def get_node_at_position(self, pos):
        for node in reversed(self.nodes): # Check topmost nodes first
            node_rect = QRectF(node['x'], node['y'], node['width'], node['height']) # Assuming these keys exist
    ...
    def process_node_graph(self, suppress_auto_save=False, changed_nodes=None):
        ...
        for node in self.nodes:
            ...
            # 清除旧的输出 (总是在开始时清除，process_node 会重新计算)
        ...
        for conn in self.connections:
            if conn['input_node'] == node:
                input_connections.append(
                    f"{conn['output_node'].get('id', 'unknown')}:{conn['output_port']}->{conn['input_port']}"
                )
    ...
    def process_node(self, node):
        ...
        if 'processed_outputs' in node:
            ...
            return node['processed_outputs'] if isinstance(node['processed_outputs'], dict) else {}
        ...
        if cached_result is not None:
            print(f"缓存命中：节点 '{node.get('title', '未知')}' ({node.get('id', '未知ID')})")
            ...
            if hasattr(self, 'performance_mode_enabled') and self.performance_mode_enabled and 'script_info' in node:
                ...
                if is_perf_sensitive:
                    ...
                    if target_size and all(size > 50 for size in target_size):  # 确保目标尺寸合理
                        ...
                        # 第二步：只有当最大图像尺寸明显大于预览窗口时才应用缩放
                        if max_width > target_size[0] * 1.2 or max_height > target_size[1] * 1.2:
                            # 计算全局缩放比例（基于最大的那个图像）
    ...
    def update_preview(self):
        ...
        if preview_node:
            # 移除这里的判断逻辑，总是调用 update_preview_from_node
    ...
    def update_preview_from_node(self, node):
        ...
        if img is not None:
            ...
            if hasattr(self, 'preview_display_widget') and qimage:
                # 注意：这里传递的是未缩放的原始 QImage
                # PreviewDisplayWidget 的 set_image 需要能处理 QImage
                # 并且在其内部根据当前的 zoom 和 pan 状态来绘制
                self.preview_display_widget.set_image(QPixmap.fromImage(qimage)) # 假设 set_image 接收 QPixmap
    ...
    def auto_save_node_graph(self):
        ...
        try:
            ...
            if self.current_image_path:
                # 基于图像名称创建文件名
                image_filename = os.path.splitext(os.path.basename(self.current_image_path))[0]
                prefix = f"{image_filename}_auto_"
            else:
                # 如果没有加载图片，使用"untitled"作为前缀
                prefix = "untitled_auto_"
            ...
            # 创建文件名
            filename = f"{prefix}{new_index}.json"
    ...
    def create_film_preview(self):
        ...
        self.film_preview_list.itemClicked.connect(self.on_film_item_double_clicked)
    ...
    def update_film_preview(self):
        ...
        # 确保工作文件夹存在
        if not hasattr(self, 'work_folder') or not self.work_folder:
            # print("工作文件夹未设置") # 可以选择性打印
            # 尝试使用默认文件夹或提示用户选择
            self.work_folder = os.path.join(os.path.expanduser("~"), "imgpp_work") # 示例默认文件夹
            if not os.path.exists(self.work_folder):
                 try:
                     os.makedirs(self.work_folder, exist_ok=True)
                     print(f"已创建默认工作文件夹: {self.work_folder}")
                 except OSError as e:
                     print(f"无法创建默认工作文件夹 {self.work_folder}: {e}")
                     # 如果连默认的都创建不了，可能需要更强的错误处理或直接返回
                     # return
            # 如果没有工作文件夹，直接返回，不显示任何内容
            # return

        if not os.path.exists(self.work_folder):
             print(f"工作文件夹不存在: {self.work_folder}")
             # 可以添加一个提示标签到 ListWidget
             placeholder_item = QListWidgetItem("未找到工作文件夹或无图片")
             placeholder_item.setFlags(Qt.NoItemFlags) # 不可选
             self.film_preview_list.addItem(placeholder_item)
             return

        # 获取工作文件夹中的图像文件
        image_exts = ['.jpg', '.jpeg', '.png', '.tif', '.tiff', '.bmp', '.webp', '.gif'] # 添加更多格式
        images = []
        try:
            for file in os.listdir(self.work_folder):
                print(file)
                file_path = os.path.join(self.work_folder, file)
            ...
            print(f"扫描工作文件夹时出错: {e}")
            return

        if not images:
            # print("工作文件夹中没有找到支持的图像文件。")
             # 添加提示标签
            placeholder_item = QListWidgetItem("无图片")
            placeholder_item.setFlags(Qt.NoItemFlags)
            self.film_preview_list.addItem(placeholder_item)
            return

        print(f"FILM_PREVIEW (Full): Found {len(images)} images. Processing...")
        thumbnail_size = self.film_preview_list.iconSize()

        processed_count = 0
        for image_file in images:
            image_path = os.path.join(self.work_folder, image_file)
            pil_img = None # 初始化 pil_img
            try:
                print(f"FILM_PREVIEW (Full): Processing {image_path}")
                pil_img = Image.open(image_path)
                # ... (thumbnail, qimage转换逻辑不变) ...
                
                # 假设 qimage 的转换逻辑在这里
                if hasattr(self, 'pil_to_qimage'):
                    qimage = self.pil_to_qimage(pil_img) # pil_img 必须先被打开
                else:
                    # 你的手动转换逻辑，确保 pil_img 已打开
                    if pil_img.mode == 'RGBA':
                        data = pil_img.tobytes('raw', 'BGRA')
                        qimage = QImage(data, pil_img.width, pil_img.height, QImage.Format_RGBA8888_Premultiplied)
                    elif pil_img.mode == 'RGB':
                        data = pil_img.tobytes('raw', 'RGB')
                        qimage = QImage(data, pil_img.width, pil_img.height, QImage.Format_RGB888)
                    elif pil_img.mode == 'L':
                        pil_img_rgb = pil_img.convert('RGB') # pil_img_rgb 是新对象
                        data = pil_img_rgb.tobytes('raw', 'RGB')
                        qimage = QImage(data, pil_img_rgb.width, pil_img_rgb.height, QImage.Format_RGB888)
                        pil_img_rgb.close() # 关闭转换后的副本
                    else: # 其他模式，尝试转换
                        pil_img_converted = pil_img.convert('RGB') # pil_img_converted 是新对象
                        data = pil_img_converted.tobytes('raw', 'RGB')
                        qimage = QImage(data, pil_img_converted.width, pil_img_converted.height, QImage.Format_RGB888)
                        pil_img_converted.close() # 关闭转换后的副本

                if qimage is None or qimage.isNull():
                     print(f"FILM_PREVIEW (Full): [{image_file}] FAILED to create valid QImage. Skipping.")
                     continue # pil_img 会在 finally 中关闭
                ...
                print(f"FILM_PREVIEW_ERROR (Full): [{image_file}] FileNotFoundError.")
            except Image.UnidentifiedImageError:
                print(f"FILM_PREVIEW_ERROR (Full): [{image_file}] UnidentifiedImageError.")
            except Exception as e:
                print(f"FILM_PREVIEW_ERROR (Full): [{image_file}] General error: {str(e)}")
                import traceback
                traceback.print_exc() # 打印完整堆栈以了解错误
            finally:
                if pil_img: #确保 pil_img 不是 None (例如 Image.open 失败时)
                    try:
                        pil_img.close()
                    except Exception as e_close:
                       pass
        # 使用 repaint (之前确认简化版用此法有效)
    ...
    def on_film_item_double_clicked(self, item: QListWidgetItem): # 指定item类型，方便代码提示
        ...
        if item:
            image_path = item.data(Qt.UserRole) # 从 UserRole 获取完整路径
            if image_path and os.path.exists(image_path):
                print(f"FILM_PREVIEW: Item double-clicked, image_path: {image_path}")
                self.open_image_path(image_path) # 这会加载图片并触发 update_film_preview
            else:
                # 如果路径无效，也打印一个警告或消息框
                QMessageBox.warning(self, "错误", f"无法打开图像，路径无效或文件不存在: {image_path}")
                print(f"FILM_PREVIEW_ERROR: Double-clicked item's image_path invalid or file does not exist: {image_path}")   
        ...
        try:
            ...
            for conn_data in data['connections']:
                ...
                if output_node_id in nodes_map and input_node_id in nodes_map:
                    ...
                    # 创建连接，但不触发节点图处理
                    # 使用直接添加连接的方式而不是调用create_connection以避免处理
                    # 检查连接是否已存在
                    exists = False
                    for conn in self.connections:
                        if (conn['output_node'] == output_node and conn['output_port'] == output_port and
                    ...
                    if not exists and not self.would_form_cycle(output_node, input_node):
                        connection = {
                            'output_node': output_node,
                            'output_port': output_port,
                            'input_node': input_node,
            ...
            # 处理节点图并更新预览 - 传递 suppress_auto_save 参数
    ...
    def parse_script_header(self, file_path):
        ...
        3. 可选第六行：SupportedFeature 列表，格式示例：
           #SupportedFeatureBoolExample:True,IntExample:32,FloatExample:1.0505
        返回值字典包含：
           - inputs: 输入参数列表（字符串列表）
           - outputs: 输出参数列表
        ...
        try:
            ...
            # --- SupportedFeature 解析（第五行及以后的任何行） ---
            supported_features = {}
            
            # 检查所有行，找到SupportedFeature标记
            for line in lines[4:]:  # 从第五行开始检查
                if line.startswith('#') and 'SupportedFeature' in line:
                    # 去掉注释符号和空格
                    sf_str = line[1:].strip()
                    
                    # 查找SupportedFeature的位置
                    sf_pos = sf_str.find('SupportedFeature')
                    if sf_pos >= 0:
                        # 提取特性部分
                        features_str = sf_str[sf_pos + len('SupportedFeature'):].strip()
                        # 处理特性键值对
                        kv_list = features_str.split(',')
                        for kv in kv_list:
                            if ':' in kv:
    ...
    def save_undo_state(self):
        ...
        for node in self.nodes:
            ...
            undo_state['nodes'].append(node_data)
        ...
        for conn in self.connections:
            ...
            undo_state['connections'].append(conn_data)
        ...
        # 添加到撤销历史
        self.undo_history.append(undo_state)
        
        # 限制历史记录长度(保留最近的20个状态)
        while len(self.undo_history) > 20:
    ...
    def undo_node_graph(self, event=None):
        """撤销到上一个自动保存的状态"""
        auto_save_files = self.get_auto_save_files()

        # 检查是否有足够的状态进行撤销
        if len(auto_save_files) < 2:
            self.task_label.setText("无可撤销的操作")
            print("无可撤销的操作")
            return

        # 获取倒数第二个文件（要恢复的状态）和最后一个文件（要删除的状态）
        state_to_load = auto_save_files[-2]['path']
        state_to_delete = auto_save_files[-1]['path']

        print(f"准备撤销: 加载 {os.path.basename(state_to_load)}, 删除 {os.path.basename(state_to_delete)}")

        # 显示加载指示器
        self.task_label.setText("正在撤销...")
        QApplication.processEvents()  # 强制更新UI

        # 加载节点图
        # 传递 suppress_auto_save=True 来阻止加载过程中的自动保存
        load_successful = self.load_node_graph_from_file(state_to_load, suppress_auto_save=True)

        # --- 修改：检查加载是否成功 ---
        if load_successful:
            # 加载成功后，删除最新的自动保存文件
            try:
                os.remove(state_to_delete)
                print(f"已删除最新的自动保存文件: {os.path.basename(state_to_delete)}")
                self.task_label.setText("已撤销到上一状态")
                ...
                # 删除失败时只打印错误，不弹窗
                print(f"删除自动保存文件时出错: {str(e)}")
                self.task_label.setText("撤销完成，但删除旧状态失败") # 提供更明确的信息

                # --- 修改：即使删除失败，加载是成功的，仍然处理和更新 ---
                # 因为状态已经成功加载，所以即使删除旧文件失败，
                # 也应该处理并显示加载的状态
                self.process_node_graph(suppress_auto_save=True)
                self.node_canvas_widget.update()
                # --------------------------------------------------
            ...
            # 加载失败
            QMessageBox.critical(self, "撤销错误", f"加载状态 '{os.path.basename(state_to_load)}' 时出错。撤销操作已中止。当前节点图可能不完整或已被清除。")
            self.task_label.setText("撤销失败")
            # --- 修改：加载失败，不执行后续处理，避免处理空图或不完整的图 ---
            # self.process_node_graph(suppress_auto_save=True) # 不处理
            # self.node_canvas_widget.update() # 也不更新画布，保持加载失败前的状态（虽然可能已被clear）
            # ------------------------------------------------

        # 注意：原先位于函数末尾的 process_node_graph 和 update 调用已被移入 if load_successful 分支
    ...
    def get_auto_save_files(self):
        """获取当前图像的自动保存文件列表，并按序号排序"""
        auto_save_files = []
        if not os.path.exists(self.temp_folder):
        ...
        for file in os.listdir(self.temp_folder):
            if file.startswith(prefix) and file.endswith(".json"):
                try:
                    # 提取序号
                    index = int(file.split("_auto_")[1].split(".")[0])
                    auto_save_files.append({'path': os.path.join(self.temp_folder, file), 'index': index})
                except (ValueError, IndexError):
                    print(f"无法解析自动保存文件名: {file}")
                    continue # 跳过格式不正确的文件

        # 按序号排序
        auto_save_files.sort(key=lambda x: x['index'])
        return auto_save_files

    def can_undo(self):
        """检查是否可以执行撤销操作"""
        auto_save_files = self.get_auto_save_files()
        return len(auto_save_files) >= 2 # 至少需要两个状态才能撤销
...
class NodeCanvasWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        for conn in self.parent_app.connections:
            ...
            painter.setPen(QPen(QColor(line_color), 2.5, Qt.SolidLine)) # 加粗线条
        ...
        if hasattr(self.parent_app, 'connecting_from') and self.parent_app.connecting_from:
            if hasattr(self.parent_app, 'connection_start_pos') and hasattr(self.parent_app, 'connection_current_pos'):
                ...
                # 设置临时线条样式
                painter.setPen(QPen(QColor(line_color), 2.5, Qt.DashLine)) # 加粗线条
