...
def process(inputs, params):
    ...
    try:
        ...
        if is_image_input:
            # 处理图像乘法（常量输入是图像）
            print(f"乘法节点：检测到图像输入：形状={constant_input.shape}，应用偏移(x={offset_x}, y={offset_y})")
            ...
            if enable_10bit_scaling:
                ...
                print(f"乘法节点：应用10bit缩放，原始值={constant_input[0, 0, 0]}，缩放后={constant_value}")
            
            print(f"乘法节点：常量值={constant_value}，图像形状={img.shape}，10bit缩放={enable_10bit_scaling}")
