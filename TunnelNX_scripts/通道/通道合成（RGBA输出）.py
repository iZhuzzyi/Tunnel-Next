# f32bmp, f32bmp_1, f32bmp_2, f32bmp_3
# f32bmp
...
def process(inputs, params):
    ...
    """
    # 1. 获取并检查输入
    im_r = inputs.get('f32bmp')
    im_g = inputs.get('f32bmp_1')
    im_b = inputs.get('f32bmp_2')
    im_a = inputs.get('f32bmp_3')
    if im_r is None or im_g is None or im_b is None or im_a is None:
        print("Error: 需要四路输入 f32bmp, f32bmp_1, f32bmp_2, f32bmp_3。")
        return {}
    # 类型检查
    for name, im in [('f32bmp', im_r), ('f32bmp_1', im_g), ('f32bmp_2', im_b), ('f32bmp_3', im_a)]:
        if not isinstance(im, np.ndarray):
            print(f"Error: 输入 {name} 不是 NumPy 数组，而是 {type(im)}。")
            return {}
    # 尺寸一致性检查
    h, w = im_r.shape[:2]
    for name, im in [('f32bmp_1', im_g), ('f32bmp_2', im_b), ('f32bmp_3', im_a)]:
